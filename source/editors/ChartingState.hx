	...
	**/
	...
	function addSongUI():Void
	{
		...
		var difficultyDropDown = new FlxUIDropDownMenuCustom(stageDropDown.x, player3DropDown.y, FlxUIDropDownMenuCustom.makeStrIdLabelArray(CoolUtil.defaultDifficulties, true), function(difficulty:String)
		{
			...
			try {
				PlayState.SONG = Song.loadFromJson(_song.song.toLowerCase() + CoolUtil.getDifficultyFilePath(), _song.song.toLowerCase());				MusicBeatState.resetState();
			} catch (e:Any) {
	...
	/* will figure this out l8r --bbpanzu
	function redo(){
